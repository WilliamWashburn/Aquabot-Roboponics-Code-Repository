...
//For using sd card on software SPI pins-----------------------------------
...
pinMode(confirmPin,OUTPUT); digitalWrite(confirmPin,LOW);
...
//-----Loop-----------------------------------------------
void loop() {
  //if the triggerPin goes HIGH, capture a photo, then set the confirmPin to HIGH for 0.5 sec and to LOW
  if(digitalRead(triggerPin) == HIGH) {
    capturePhoto();
    digitalWrite(confirmPin, HIGH);delay(500);digitalWrite(confirmPin,LOW);
  }
}
//--------------------------------------------------------
...
while ( length-- )
{
  ...
  else if ((temp == 0xD8) & (temp_last == 0xFF))
  {
    ...
    if (!outFile.open(str,O_WRITE | O_CREAT | O_TRUNC)) 
    {
